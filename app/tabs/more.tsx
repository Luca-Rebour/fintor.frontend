...
type CategoryWithTotals = CategoryModel & {
  totalSpent: number;
};
...
export default function MoreTabScreen() {
  ...
  async function loadCategoriesAndTransactions(showLoader = true) {
    try {
      if (showLoader) {
        setIsLoading(true);
      }
  ...
  const categoriesWithTotals = useMemo<CategoryWithTotals[]>(() => {
    const expenseByCategoryName = new Map<string, number>();
    ...
    for (const transaction of transactions) {
      ...
      const currentAmount = expenseByCategoryName.get(key) ?? 0;

      expenseByCategoryName.set(key, currentAmount + Math.abs(transaction.amount));
    }

    return categories
      .map((category) => {
        const key = category.label.trim().toLowerCase();
        const totalSpent = expenseByCategoryName.get(key) ?? 0;
  ...
  async function handleRefresh() {
    ...
    await loadCategoriesAndTransactions(false);
    ...
    return (
      <View className="flex-1 items-center justify-center bg-app-bgPrimary px-6">
        ...
        <Pressable
          ...
          onPress={() => loadCategoriesAndTransactions()}
        >
